...
\hypersetup{
  ...
  pdfsubject={Triangle packing and clique partitions in split graphs},
  pdfkeywords={clique partition, split graph, triangle packing, fractional packing, factorization, polarization}
}
\urlstyle{same}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\itshape Linear-Error Clique Partitions of Split Graphs}
...
\begin{center}
\small
\textbf{Frozen author manuscript: v1.0.4}\\
July 3, 2026\\
Final closed author version; all internal proof steps are analytic, with external theorem dependencies isolated in Section 2.4; supplementary computations are non-logical regression tests; extensive multi-model AI-assisted adversarial checking completed; not yet independently peer-reviewed or journal-refereed.\\[0.4em]
\textbf{MSC 2020:} Primary 05C70; Secondary 05C35, 05C72, 05C15.
\end{center}}
\begin{document}
\begin{titlepage}
\centering
\vspace*{2.2cm}
{\sffamily\bfseries\fontsize{22}{27}\selectfont Linear-Error Clique Partitions of Split Graphs via Structured Triangle Packing\par}
\vspace{1.5cm}
...
\medskip
\noindent\textbf{Keywords:} clique partition; split graph; triangle packing; fractional packing; factorization; polarization; list edge coloring; graph decomposition.
\end{abstract}
\vspace{0.8em}
\section{1. Introduction}\label{introduction}

\subsection{1.1 Clique partitions of split graphs}\label{clique-partitions-of-split-graphs}
...
\subsection{1.2 Triangle packings and the linear-error target}\label{triangle-packings-and-the-linear-error-target}
...
\subsubsection{Theorem 1.1 --- Linear-error split-graph theorem}\label{theorem-1.1-linear-error-split-graph-theorem}
...
\subsubsection{Corollary 1.2 --- Linear-error clique partition bound}\label{corollary-1.2-linear-error-clique-partition-bound}
...
\subsection{1.3 Localization of hypothetical obstructions}\label{localization-of-hypothetical-obstructions}

\subsubsection{Proposition 1.3 --- Localization of hypothetical obstructions}\label{proposition-1.3-localization-of-hypothetical-obstructions}
...
\subsection{1.4 What the theorem does not prove}\label{what-the-theorem-does-not-prove}

Let \(\nu_3^*(G)\) denote the fractional triangle-packing number. The present proof does \textbf{not} establish
...
\item
  \textbf{Bulk:} \(\alpha\) stays away from both \(0\) and \(2\). An exact common-profile LP and a replication argument produce quadratic fractional slack. Haxell--Rödl converts the fractional packing into an integral one with only \(o(n^2)\) loss.
\item
  \textbf{Sparse independent side:} \(\alpha\to0\). Each independent vertex anchors a large matching inside its neighborhood. The residual clique is almost complete. After deleting \(O(p)\) edges to correct divisibility, it admits an exact triangle decomposition.
\item
  \textbf{Near-extremal corridor:} \(\alpha\to2\). Write \(q=2p-s\). Averaged factorization closes \(s=O(\sqrt p)\). For \(\sqrt p\ll s=o(p)\), high dispersion is paid by a double-factor polarization term, while low dispersion implies closeness to a common center and is handled by shifted-center gain completion.
\end{enumerate}

\subsection{1.6 Supplementary computational audits}\label{supplementary-computational-audits}
...
\subsection{2.3 Complete graph factorizations}\label{complete-graph-factorizations}
...
\subsubsection{Theorem 2.1 --- Haxell--Rödl/Yuster}\label{theorem-2.1-haxellruxf6dlyuster}
...
\subsubsection{Theorem 2.2 --- Borodin--Kostochka--Woodall}\label{theorem-2.2-borodinkostochkawoodall}
...
\subsubsection{Theorem 2.3 --- Dense triangle decomposition}\label{theorem-2.3-dense-triangle-decomposition}
...
\section{3. The Common-Profile Linear Program}\label{the-common-profile-linear-program}
...
\subsection{3.1 Symmetric cover variables}\label{symmetric-cover-variables}
...
\subsubsection{Theorem 3.1 --- Common-profile formula}\label{theorem-3.1-common-profile-formula}
...
\subsection{3.3 Interpretation of the three covers}\label{interpretation-of-the-three-covers}
...
\tightlist
\item
  The \textbf{uniform cover} assigns weight \(1/3\) to every clique edge and weight \(1/3\) to every independent incidence.
\item
  The \textbf{separated cover} assigns full weight to the internal edges of \(N\) and \(R\), with no crossing weight.
\item
  The \textbf{hot-neighborhood cover} assigns full weight to \(E(N)\) and uniform weight to the remainder.
\end{itemize}
...
\section{4. Replication and the Exact Fractional Margin}\label{replication-and-the-exact-fractional-margin}

\subsection{4.1 Replication}\label{replication}

\subsubsection{Lemma 4.1 --- Replication bound}\label{lemma-4.1-replication-bound}
...
\subsubsection{Theorem 4.2 --- Unified fractional margin}\label{theorem-4.2-unified-fractional-margin}
...
\section{\texorpdfstring{5. Factorization Rounding Near \(q=2p\)}{5. Factorization Rounding Near q=2p}}\label{factorization-rounding-near-q2p}
...
\subsubsection{Lemma 5.2 --- Double-factor inequality}\label{lemma-5.2-double-factor-inequality}
...
\subsubsection{Lemma 6.1 --- Polarization inequality}\label{lemma-6.1-polarization-inequality}
...
\section{7. Shifted-Center Gain Completion}\label{shifted-center-gain-completion}
...
\subsection{\texorpdfstring{7.1 Packing inside \(Q\)}{7.1 Packing inside Q}}\label{packing-inside-q}
...
\subsection{\texorpdfstring{7.3 Completion inside \(R\)}{7.3 Completion inside R}}\label{completion-inside-r}
...
\subsubsection{Lemma 7.1 --- Reserved-gain shifted-center inequality}\label{lemma-7.1-reserved-gain-shifted-center-inequality}
...
\section{8. The Sparse-Independent Regime}\label{the-sparse-independent-regime}

Assume \(q=o(p)\). We prove the required bound directly and integrally.

\subsection{8.1 Minimal-counterexample degree bound}\label{minimal-counterexample-degree-bound}
...
\subsection{\texorpdfstring{9.2 The endpoint \(\alpha_k\to0\)}{9.2 The endpoint \textbackslash alpha\_k\textbackslash to0}}\label{the-endpoint-alpha_kto0}
...
\subsection{\texorpdfstring{9.3 The endpoint \(\alpha_k\to2\)}{9.3 The endpoint \textbackslash alpha\_k\textbackslash to2}}\label{the-endpoint-alpha_kto2}
...
\subsection{10.2 Sharpness of the leading term}\label{sharpness-of-the-leading-term}
...
\subsection{10.3 Localization of difficult sequences}\label{localization-of-difficult-sequences}
...
\subsection{10.4 Exact common-profile fractional value}\label{exact-common-profile-fractional-value}
...
\subsection{\texorpdfstring{10.5 Localized effectivity near \(q=2p\)}{10.5 Localized effectivity near q=2p}}\label{localized-effectivity-near-q2p}

\subsubsection{Proposition 10.1 --- Explicit corridor bounds}\label{proposition-10.1-explicit-corridor-bounds}
...
\subsection{11.1 From fractional asymptotics to a linear-error integral theorem}\label{from-fractional-asymptotics-to-a-linear-error-integral-theorem}
...
\subsection{11.2 Why residual maximum degree is not the right invariant}\label{why-residual-maximum-degree-is-not-the-right-invariant}
...
\subsection{11.3 Localized effectivity and remaining non-effectivity}\label{localized-effectivity-and-remaining-non-effectivity}
...
\tightlist
\item
  The Haxell--Rödl theorem is used through an asymptotic \(o(n^2)\) statement in the bulk.
\item
  Dense triangle decomposition theorems are invoked with unspecified sufficiently-large thresholds in the sparse-independent regime.
\end{enumerate}

Thus no hidden non-effectivity remains in Sections 5--7; it comes only from the two external asymptotic inputs above.

\subsection{11.4 Relationship with the fractional extremal theorem}\label{relationship-with-the-fractional-extremal-theorem}
...
\subsection{11.5 Relationship with the chordal linear-error problem}\label{relationship-with-the-chordal-linear-error-problem}
...
\section{12. Potential Uses and Future Directions}\label{potential-uses-and-future-directions}

\subsection{12.1 Universal linear integrality gap}\label{universal-linear-integrality-gap}
...
\subsection{12.2 Effective global constants}\label{effective-global-constants}
...
\subsection{12.4 Stability and extremal classification}\label{stability-and-extremal-classification}
...
\subsection{12.5 Higher-clique linear-error packing}\label{higher-clique-linear-error-packing}
...
\subsection{12.6 Chordal and clique-tree applications}\label{chordal-and-clique-tree-applications}
...
\subsection{12.7 Reusable proof principle}\label{reusable-proof-principle}
...
\section{13. Reproducibility and closure package}\label{reproducibility-and-closure-package}
...
\subsection{The included script is a supplementary regression test for the explicit corridor constants. Its output is not used as a hypothesis, reduction, or proof step anywhere in the manuscript. Historical exploratory computations are not part of the logical proof package.}\label{the-included-script-is-a-supplementary-regression-test-for-the-explicit-corridor-constants.-its-output-is-not-used-as-a-hypothesis-reduction-or-proof-step-anywhere-in-the-manuscript.-historical-exploratory-computations-are-not-part-of-the-logical-proof-package.}

\section{Appendix A. Algebra of the Fractional Margin}\label{appendix-a.-algebra-of-the-fractional-margin}
...
\section{Appendix B. Divisibility Correction}\label{appendix-b.-divisibility-correction}
...
\section{Appendix C. Computational Audits}\label{appendix-c.-computational-audits}

\subsection{C.1 Exact common-profile LP}\label{c.1-exact-common-profile-lp}

\texttt{verify\_common\_profile\_lp.py} enumerates the vertices of the symmetrized cover polyhedron using exact rational Gaussian elimination and compares the optimum with (3.5).

\subsection{C.2 Exact fractional margin}\label{c.2-exact-fractional-margin}
...
\texttt{verify\_factor\_rounding.py} and \texttt{verify\_shifted\_center.py} compute exact integral triangle packings on small instances using a binary edge-capacity ILP. They verify the stated upper bounds on \(\Phi(G)\).

\subsection{C.4 Polarization}\label{c.4-polarization}

\texttt{verify\_polarization.py} performs exhaustive small-profile and randomized exact-integer checks of Lemma 6.1.

\subsection{C.5 Divisibility}\label{c.5-divisibility}

\texttt{verify\_divisibility\_correction.py} exhaustively verifies the path parity construction through order eighteen, checks the fixed minimum-degree threshold exactly, and follows the full parity and modulo-three correction on randomized dense residuals.
...
{[}1{]} B. Barber, D. Kühn, A. Lo, and D. Osthus, \textbf{Edge-Decompositions of Graphs with High Minimum Degree}, \emph{Advances in Mathematics} \textbf{288} (2016), 337--385. DOI: \texttt{10.1016/j.aim.2015.09.032}.

{[}2{]} O. V. Borodin, A. V. Kostochka, and D. R. Woodall, \textbf{List Edge and List Total Colourings of Multigraphs}, \emph{Journal of Combinatorial Theory, Series B} \textbf{71} (1997), no. 2, 184--204. DOI: \texttt{10.1006/jctb.1997.1780}.

{[}3{]} G.-T. Chen, P. Erdős, and E. T. Ordman, \textbf{Clique Partitions of Split Graphs}, in \emph{Combinatorics, Graph Theory, Algorithms and Applications} (Beijing, 1993), World Scientific, 1994, 21--30.

{[}4{]} G. A. Dirac, \textbf{Some Theorems on Abstract Graphs}, \emph{Proceedings of the London Mathematical Society} (3) \textbf{2} (1952), 69--81. DOI: \texttt{10.1112/plms/s3-2.1.69}.

{[}5{]} F. Dross, \textbf{Fractional Triangle Decompositions in Graphs with Large Minimum Degree}, \emph{SIAM Journal on Discrete Mathematics} \textbf{30} (2016), no. 1, 36--42. DOI: \texttt{10.1137/15M1014310}.

{[}6{]} P. Erdős, E. T. Ordman, and Y. Zalcstein, \textbf{Clique Partitions of Chordal Graphs}, \emph{Combinatorics, Probability and Computing} \textbf{2} (1993), no. 4, 409--415. DOI: \texttt{10.1017/S0963548300000808}.

{[}7{]} P. E. Haxell and V. Rödl, \textbf{Integer and Fractional Packings in Dense Graphs}, \emph{Combinatorica} \textbf{21} (2001), 13--38. DOI: \texttt{10.1007/s004930170003}.

{[}8{]} J. P. Traverso Gianini, \textbf{Complete-Split Extremizers for Fractional Clique Parameters on Chordal Graphs}, author manuscript, v0.7-pre, July 2026.

{[}9{]} R. Yuster, \textbf{Integer and Fractional Packing of Families of Graphs}, \emph{Random Structures \& Algorithms} \textbf{26} (2005), 110--118. DOI: \texttt{10.1002/rsa.20048}.

{[}10{]} P. Keevash, \textbf{The Existence of Designs}, arXiv:\texttt{1401.3665}, revised 2018.

{[}11{]} P. Allen, J. Böttcher, O. Cooley, and R. Mycroft, \textbf{Tight Cycles and Regular Slices in Dense Hypergraphs}, \emph{Journal of Combinatorial Theory, Series A} \textbf{149} (2017), 30--100. DOI: \texttt{10.1016/j.jcta.2017.01.003}.
